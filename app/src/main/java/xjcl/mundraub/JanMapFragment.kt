...
class JanMapFragment : SupportMapFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        mapView.post {
            ...
            val bmpSample = BitmapFactory.decodeResource(
                resources,
                R.drawable.otherfruit
            )
            Log.e("scrHeight", scrHeight.toString())
            Log.e(
                "bmp wxh",
                " " + bmpSample.width + " " + bmpSample.height
            )
            ...
            infoBar.layoutParams = RelativeLayout.LayoutParams(
            ...
            infoBar.background = materialDesignBg(
                (7.5 * density).toInt(),
                (2.5 * density).toInt(),
                999F
            )
            ...
            fun animateJump(iv : View) {
                ...
                iv.animate().y((origY[iv]?:0F) - 6 * density).withEndAction { iv.animate().y(
                    origY[iv]?:0F) }  // We commence to make you (jump, jump)! :D
            }
            fun handleClick(iv : View, key : Int, cond : (Int) -> Boolean) {
                ...
                for (other in ivs)
                    other.value.setColorFilter(
                        Color.parseColor(
                            if (cond(other.key) || other.key >= 90) "#FFFFFF" else "#555555"
            ...
            for (group in groups) {
                ...
                Log.e(
                    "tvm",
                    "${tv.measuredHeight} ${tv.measuredWidth}"
                )
                ...
                val tvHolder =
                    RelativeLayout(context)   // NEEDED else the touch regions are all wrong
                tvHolder.layoutParams = LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.WRAP_CONTENT,
                    LinearLayout.LayoutParams.WRAP_CONTENT
                ).apply {
                ...
                totalLeftPadding =
                    max(
                        totalLeftPadding,
                        (.04 * scrHeight).toInt() + bmpSample.width + tv.measuredHeight
                    )
            }
            ...
            ivs[98]?.apply {
                this.setOnClickListener {
                    val set = treeIdToSeason.keys.filter {
                        isSeasonal(
                            it,
                            getCurMonth()
                        )
                ...
                fillImageView(this,
                    R.drawable._marker_season_filter_b, i)
                linear.addView(this)
            }
            ...
            ivs[99]?.apply {
                ...
                fillImageView(this,
                    R.drawable._marker_reset_filter_a, i + 1)
                linear.addView(this)
            }
            ...
            val linearHolder = LinearLayout(context).apply {
                ...
                background = materialDesignBg(
                    pad,
                    pad,
                    bmpSample.width / 2F + pad
                )
                // https://developer.android.com/training/material/shadows-clipping
            ...
            fab.setImageBitmap(
                BitmapFactory.decodeResource(
                    resources,
                    R.drawable.material_directions
                )
            )
            ...
            fab.layoutParams = CoordinatorLayout.LayoutParams(
